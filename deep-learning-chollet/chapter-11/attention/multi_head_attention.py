"""
~/opt/anaconda3/envs/deepred/lib/python3.9/site-packages/keras/layers/multi_head_attention.py
"""
...
def _build_attention_equation(rank, attn_axes):
  ...
  print(f"Dot product attention equation: {dot_product_equation}")
  print(f"Combine equation: {combine_equation}")
...
def _build_proj_equation(free_dims, bound_dims, output_dims):
  ...
  print(f"Proj equation: {equation}")
  print(f"Input str: {input_str}")
  print(f"Kernel str: {kernel_str}")
  print(f"Output str: {output_str}")
  print(f"Bias axes: {bias_axes}")
  print(f"len output: {len(output_str)}")
  return equation, bias_axes, len(output_str)


def _get_output_shape(output_rank, known_last_dims):
  print(f"output rank: {output_rank}")
  print(f"known last dims: {known_last_dims}")
  print(f"list known last dims: {list(known_last_dims)}")
...
class MultiHeadAttention(Layer):
  ...
  def __init__(self,
    ...
    print(f"init output shape: {output_shape}")
  ...
  def _build_from_signature(self, query, value, key=None):
    ...
    with tf_utils.maybe_init_scope(self):
      print(f"Signature query shape: {self._query_shape}")
      print(f"Query rank: {self._query_shape.rank}")
      free_dims = self._query_shape.rank - 1
      print(f"query Free dims: {free_dims}")
      einsum_equation, bias_axes, output_rank = _build_proj_equation(
          free_dims, bound_dims=1, output_dims=2)
      print(f"Output shape: {_get_output_shape(output_rank - 1, [self._num_heads, self._key_dim])}")
      self._query_dense = einsum_dense.EinsumDense(
          ...
          name="query",
          **common_kwargs)
      print(f"key Free dims: {self._key_shape.rank - 1}")
      ...
      self._key_dense = einsum_dense.EinsumDense(
          ...
          name="key",
          **common_kwargs)
      print(f"Value Free dims: {self._value_shape.rank - 1}")
  ...
  def _make_output_dense(self, free_dims, common_kwargs, name=None):
    ...
    if self._output_shape:
      ...
      print(f"output shape last dim: {[self._query_shape[-1]]}")
      output_shape = [self._query_shape[-1]]
    print("output")
    einsum_equation, bias_axes, output_rank = _build_proj_equation(
        free_dims, bound_dims=2, output_dims=len(output_shape))
    print(f"output shape: {_get_output_shape(output_rank - 1, output_shape)}")
  ...
  def _build_attention(self, rank):
    ...
    print(f"Attention axes: {self._attention_axes}")
    self._dot_product_equation, self._combine_equation, attn_scores_rank = (
        _build_attention_equation(rank, attn_axes=self._attention_axes))
    norm_axes = tuple(
        range(attn_scores_rank - len(self._attention_axes), attn_scores_rank))
    print(f"Norm axes: {norm_axes}")
  ...
  def call(self,
    ...
    print(f"Dense query shape: {query.shape}")
    print(f"Dense value shape: {value.shape}")
    print(f"Dense key shape: {key.shape}")
